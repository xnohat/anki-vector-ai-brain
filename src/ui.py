...
class UserInterface:
	...
	def speak(self, event=None) -> None:
		text = self.chat_entry.get().strip()
		self.chat_entry.delete(0, END)
		if not text:
			return
		# Hand the typed text to the brain (set by app.py). Lets you talk to
		# Vector by typing when no microphone is attached.
	...
	def construct_ui(self) -> None:
		...
		self.chat_entry.bind("<Return>", self.speak)   # Enter sends
	...
	def __init__(self) -> None:
		...
		self.frame = None
		self.on_submit = None   # app.py sets this to route typed text to the brain
