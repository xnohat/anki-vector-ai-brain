...
class VectorBot:
    ...
    def __del__(self) -> None:
        # Best-effort cleanup; ignore if already disconnected or loop is gone.
        try:
...
class Action:
    ...
    @latency
    def say_wav(self, path: str, volume: int = 70) -> bool:
        """Stream a pre-rendered WAV (e.g. Vietnamese TTS) through Vector's speaker.

        Returns True on success. Waits for playback to finish (AsyncRobot future).
        """
        try:
            result = self.robot.audio.stream_wav_file(path, volume)
            if isinstance(result, concurrent.futures.Future):
                result.result()  # block until playback completes
            return True
        except Exception:
            print(traceback.format_exc())
            return False
